    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        for teamLabel in labelArray{
            UIView.animate(withDuration: 2, delay: TimeInterval(0+i), options: [.curveLinear], animations: {
                teamLabel.frame =  CGRect(x: 20, y: 30*i, width: 80, height: 30)
            })
            
            UIView.animate(withDuration: 2.4, delay: 0.8,options: [UIViewAnimationOptions.curveEaseOut], animations: {
                self.labelArray[i].alpha = 1.0
            }, completion: nil)
